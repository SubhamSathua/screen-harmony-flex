    ...
    fun generate12WordPhrase(): List<String> {
        ...
        while (words.size < 12) {
            ...
            if (selectedIndices.add(idx)) {
                words.add(WORD_LIST[idx])
            }
        }
        return words
    }

    fun hashPhrase(phraseWords: List<String>): String {
        val normalized = phraseWords.joinToString(" ") { it.trim().lowercase() }
        val digest = MessageDigest.getInstance("SHA-256")
        val bytes = digest.digest(normalized.toByteArray(Charsets.UTF_8))
        return bytes.joinToString("") { "%02x".format(it) }
    }

    fun validatePhrase(input: String): Boolean {
        val words = input.trim().split(Regex("\\s+")).filter { it.isNotBlank() }
        if (words.size != 12) return false
        return words.all { WORD_LIST.contains(it.lowercase()) }
    }
}
